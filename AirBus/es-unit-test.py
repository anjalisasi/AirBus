...
class ElDocumentAPITest(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        mylines = []
        with open ('/tmp/tempfile', 'rt') as myfile:
            for myline in myfile:
                mylines.append(myline.rstrip('\n'))

        AUTH_USERNAME = "elastic"
        AUTH_PASSWORD = mylines[1]
        PORTNO = mylines[2]
        HOSTNAME = mylines[0]
        auth= None
        ...
        cls.es = Elasticsearch(
                [{'host': HOSTNAME , 'port': PORTNO,'http_auth': auth}])
    ...
    def test_create_existing_id(self):
        '''Check that a 409 response returns if document with existing ID
         is created'''
        self.es.create(
            index='twitter',
            id=1,
    ...
    def test_index_existing_doc(self):
        ''' Unlike 'es.create', if the document already exists, updates the document
    ...
    def test_source_exists(self):
        '''Check that the specified JSON document has source'''
        self.es.index(
            index='eklmn',
            id=1,
            body={
        ...
        self.assertEqual(
            self.es.exists_source(
                index='eklmn',
                id=1),
            True)

    def test_doc_not_found(self):
        '''Check that NotFoundError is returned if the requested document
         doesn't exist in an index'''
        self.es.index(
            index='twitter',
            id=1,
            body={
    ...
    def test_get_source(self):
        '''Check that the source of a JSON document is retrieved
         from an index'''
        self.es.index(
            index='twitter',
            id=1,
    ...
    def test_delete_by_query(self):
        '''Check that document gets deleted by query. Refresh is set to 'true'
         to make created document visible to search'''
        self.es.index(
            index='rick&morty',
            id=1,
            body={
    ...
    def test_fail_delete_by_query(self):
        '''Check that delete operation fails if query doesn't match
         any document'''
        self.es.index(
            index='rick&morty',
            id=1,
    ...
    def test_update_by_query(self):
        '''Check that document gets updated by query. Refresh is set to 'true'
         to make created document visible to search'''
        self.es.index(
            index='test',
            id=1,
    ...
    def test_mget(self):
        ...
        If there is a failure getting a particular document,
        the error is included in place of the document.'''
    ...
    def test_bulk_index(self):
        '''Check API performs multiple indexing or delete operations
    ...
    def test_temvestors(self):
        '''Checks that expected information and statistics for terms
         in the field of a document correspond actual'''
    ...
    def test_temvestors_not_found(self):
        '''Response body contains 'found': 'False' if document is not
         present'''
    ...
    def test_temvestors_invalid_field(self):
        ''' 'term_vectors' is an empty array if field is not present
         in a document'''
    ...
    def test_mtermverctors_not_found(self):
        '''Response body contains 'found': 'False' if document is
         not present'''
